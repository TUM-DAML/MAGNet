...
class GoalDirectedWrapper(GoalDirectedGenerator):
    ...
    def get_training_smiles(self):
        smiles = smiles_from_file(BASELINE_DIR / "smiles_files" / self.dataset / "train.txt")
    ...
    def optimize_molecules_with_mso(
        ...
        optimized_molecules = []
        with tqdm(
            total=number_molecules, desc=scoring_function.name, disable=disable_progress_bar
        ) as progress_bar:
