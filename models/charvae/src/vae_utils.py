...
import baselines.charvae.src.hyperparameters as hyperparameters
import baselines.charvae.src.mol_utils as mu
from baselines.charvae.src.models import (
    load_decoder,
    load_encoder,
    load_property_predictor,
)
from baselines.charvae.src.mol_utils import fast_verify
...
class VAEUtils(object):
    def __init__(self, encoder_file=None, decoder_file=None, config=None, dataset=None, seed=None, id=None):
        ...
        if encoder_file is not None:
            self.params["encoder_weights_file"] = (
                BASELINE_DIR / "model_ckpts" / "CHARVAE" / dataset / id / encoder_file
            )
        if decoder_file is not None:
            self.params["decoder_weights_file"] = (
                BASELINE_DIR / "model_ckpts" / "CHARVAE" / dataset / id / decoder_file
            )
        # char stuff
