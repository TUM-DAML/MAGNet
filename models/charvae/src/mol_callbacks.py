...
class WeightAnnealer_epoch(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        assert type(new_weight) == float, 'The output of the "schedule" function should be float.'
...
class EncoderDecoderCheckpoint(ModelCheckpoint):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.encoder.save(os.path.join(self.p["checkpoint_path"], self.p["encoder_weights_file"]))
        self.decoder.save(os.path.join(self.p["checkpoint_path"], self.p["decoder_weights_file"]))
