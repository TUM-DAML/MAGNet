...
from keras.layers import GRU, BatchNormalization, Concatenate, Input, Lambda
...
def load_encoder(params):
    ...
    print(params["encoder_weights_file"])
...
def decoder_model(params):
    ...
    true_seq_in = Input(shape=(params["MAX_LEN"], params["NCHARS"]), name="decoder_true_seq_input")
...
def variational_layers(z_mean, enc, kl_loss_var, params):
    ...
    def sampling(args):
        ...
        epsilon = tf.random.normal(shape=(params["batch_size"], params["hidden_dim"]), mean=0.0, stddev=1.0)
        # insert kl loss here
...
def property_predictor_model(params):
    ...
    prop_mid = Dense(params["prop_hidden_dim"], activation=params["prop_pred_activation"])(ls_in)
