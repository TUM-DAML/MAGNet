...
from baselines.psvae.data.bpe_dataset import BPEMolDataset, get_dataloader
from baselines.psvae.evaluation.utils import PROPS, get_normalized_property_scores
from baselines.psvae.modules.encoder import Encoder
from baselines.psvae.modules.predictor import Predictor
from baselines.psvae.modules.vae_piece_decoder import VAEPieceDecoder
...
class PSVAEModel(pl.LightningModule):
    ...
    def cal_beta(self):
        ...
        if step < warmup:
            ...
            step_beta, anneal_iter = (
                self.config["step_beta"],
                self.config["kl_anneal_iter"],
            )
            beta = min(
                self.config["max_beta"],
                ((step - warmup) // anneal_iter + 1) * step_beta,
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.weighted_loss(
            pred_loss, rec_losses[0], rec_losses[-1]
        )  # first is overall rec loss, last is kl loss
