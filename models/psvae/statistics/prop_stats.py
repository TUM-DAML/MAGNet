...
from baselines.psvae.evaluation.utils import get_normalized_property_scores, restore_property_scores, PROPS
from baselines.psvae.utils.chem_utils import smiles2molecule
...
def parse():
    ...
    parser.add_argument("--data", type=str, required=True, help="Path of data")
    parser.add_argument(
        "--output_dir", type=str, default=DEFAULT_OUTPUT_DIR, help="Path to save statistic figures and log"
    )
    parser.add_argument(
        "--cpus", type=int, default=os.cpu_count(), help="Number of cpu cores to parallel calculation"
    )
...
def draw_hist(data, title, xlabel, ylabel="Frequency", png=None):
    plt.clf()
    plt.hist(data, bins=40, density=False, facecolor="blue", edgecolor="black", alpha=0.3)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
    if png is None:
...
def main(args):
    ...
    progs = [
        pool.apply_async(
            get_props, args=(smiles[i : min(len(smiles), i + size)], [func for _, func in addition_props])
        )
