...
import baselines.psvae.pl_models
from baselines.psvae.data.dataset import get_vocab_dataset
...
def parse():
    ...
    parser.add_argument("--data_path", type=str, required=True, help="path of input data")
    parser.add_argument("--vocab_path", type=str, required=True, help="path of vocabulary")
    parser.add_argument("--ckpt", type=str, required=True, help="path of checkpoint of model")
    parser.add_argument("--model", type=str, choices=["vae_dgmg", "dgmg"], required=True, help="type of model")
    parser.add_argument("--batch_size", type=int, default=128, help="size of mini-batch")
    parser.add_argument("--num_workers", type=int, default=4, help="number of cpus to use")
    parser.add_argument("--save_vectors", action="store_true", help="store all the vectors")
    parser.add_argument(
        "--output_dir", type=str, default=OUT_DIR, help=f"path to store output files, default {OUT_DIR}"
    )
...
def draw2D(np_array):
    ...
    print(
        f"2D illustration of encoding space is saved to {FIGURE}.\n"
...
def mean_var(np_array):
    ...
    print(
        "Mean and standard variance of each dimension have been "
...
def max_min(np_array):
    ...
    print("Max and min of each dimension have been " f"saved to {MAX_MIN} with array shape {max_min.shape}")
...
def sample(n=1, method="uniform"):
    sample_funcs = {"uniform": uni_sample, "gaussian": gauss_sample}
