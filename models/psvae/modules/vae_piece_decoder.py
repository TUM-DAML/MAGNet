...
from baselines.psvae.modules.common_nn import MLP
from baselines.psvae.modules.encoder import Encoder
from baselines.psvae.utils.chem_utils import (
    cnt_atom,
    cycle_check,
    smiles2molecule,
    valence_check,
...
class VAEPieceDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.graph_embedding = Encoder(
            node_dim, num_edge_type, node_hidden_dim, dim_out=1, t=t
        )  # dim_out is of no use
        # edge link predictor
    ...
    def forward(
        ...
        src_embedding = torch.repeat_interleave(node_embedding, node_num, dim=1).view(
            batch_size, node_num, node_num, -1
        )
        ...
        dst_embedding = torch.repeat_interleave(node_embedding, node_num, dim=0).view(
            batch_size, node_num, node_num, -1
        )
        dst_embedding = dst_embedding[edge_select]
        # [batch_size, N, N, latent_dim]
        latent_repeat = torch.repeat_interleave(z_vecs, node_num**2, dim=0).view(
            batch_size, node_num, node_num, -1
        )
        ...
        if return_accu:
            ...
            piece_accu = (
                (torch.argmax(pred_piece, dim=-1) == gold_piece) & not_pad
            ).sum().item() / not_pad.sum().item()
    ...
    def inference(self, z, max_atom_num, add_edge_th, temperature):
        ...
        if edge_pred_in.shape[0]:  # maybe only one piece is generated -> no edges need to be predicted
            ...
            possible_edge_idx = [
                i for i in range(len(pred_edge)) if confidence[i] >= add_edge_th and edge_type[i] != none_bond
            ]
            ...
            for i in sorted_idx:
                ...
                if valence_check(
                    ...
                    edge_sets[end],
                    bond_type,
                    self.tokenizer.chem_vocab,
                ) and cycle_check(begin, end, gen_mol):
        ...
        if edge_pred_in.shape[0]:  # maybe only one piece is generated -> no edges need to be predicted
            ...
            for i in sorted_idx:
                ...
                if valence_check(
                    ...
                    gen_mol.GetAtomWithIdx(end).GetFormalCharge(),
                ) and cycle_check(begin, end, gen_mol):
                    gen_mol.AddBond(begin, end, self.tokenizer.chem_vocab.idx_to_bond(bond_type))
                    edge_sets[begin].append(bond_type)
                    edge_sets[end].append(bond_type)
