...
class GraphPropertyPredictor(tf.keras.layers.Layer):
    ...
    def build(self, input_shapes: GraphPropertyPredictorInput):
        with tf.name_scope(self._name):
            ...
            self._property_predictor.build(tf.TensorShape((None, self._params["graph_representation_size"])))
    ...
    @tf.function(
        input_signature=(
            GraphPropertyPredictorInput(
                node_representations=tf.TensorSpec(shape=tf.TensorShape((None, None)), dtype=tf.float32),
    ...
    def compute_epoch_metrics(self, task_results: List[Any]) -> Tuple[float, str]:
        total_num_samples = np.sum(batch_task_result["num_samples"] for batch_task_result in task_results)
