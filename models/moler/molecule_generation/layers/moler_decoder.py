...
from models.moler.molecule_generation.chem.molecule_dataset_utils import BOND_DICT
from models.moler.molecule_generation.chem.motif_utils import (
    MotifVocabulary,
    find_motifs_from_vocabulary,
)
from models.moler.molecule_generation.chem.rdkit_helpers import (
    compute_canonical_atom_order,
    get_atom_symbol,
)
from models.moler.molecule_generation.utils.decoder_batching import batch_decoder_states
from models.moler.molecule_generation.utils.moler_decoding_utils import (
    DecoderSamplingMode,
...
class MoLeRDecoder(tf.keras.layers.Layer):
        ...
        **kwargs,
        ...
        if self.has_first_node_type_selector:
            ...
            self._carbon_type_idx = next(
                type_idx for type_idx, type_name in self._index_to_node_type_map.items() if type_name == "C"
            )
        ...
        if self.uses_categorical_features:
            if "categorical_features_embedding_dim" in self._params:
                ...
                self._node_categorical_features_embedding = np.eye(node_categorical_num_classes, dtype=np.float32)

    def _batch_decoder_states(self, **kwargs) -> Generator[Tuple[Dict[str, Any], List[MoLeRDecoderState]], None, None]:
    ...
    def build(self, tensor_shapes: MoLeRDecoderInput):
        ...
        edge_candidate_representation_size = (
            input_molecule_representation_size + self._params["graph_repr_size"] + 2 * node_repr_size + edge_feature_dim
        )
        with tf.name_scope("edge_candidate_scorer"):
            self._edge_candidate_scorer.build(tf.TensorShape((None, edge_candidate_representation_size)))
        ...
        if self.has_first_node_type_selector:
            with tf.name_scope("first_node_type_selector"):
                self._first_node_type_selector.build(input_shape=[None, input_molecule_representation_size])
        ...
        if self.uses_motifs:
            with tf.name_scope("attachment_point_selector"):
                self._attachment_point_selector.build(
                    input_shape=[
                        None,
                        self._params["graph_repr_size"] + input_molecule_representation_size + node_repr_size,
                    ]
                )
    ...
    def calculate_node_and_graph_representations(
        ...
        if self.uses_categorical_features:
            ...
            initial_node_features = tf.concat([node_features, embedded_categorical_features], axis=-1)
        else:
        ...
        if self.uses_motifs:
            ...
            nodes_to_set_in_focus_bit = tf.concat([graph_to_focus_node_map, candidate_attachment_points], axis=0)
        else:
    ...
    def pick_node_type(
        ...
        return self._node_type_selector(original_and_calculated_graph_representations, training=training)
    ...
    def pick_edge(
        ...
        valid_target_to_graph_map = tf.gather(node_to_graph_map, candidate_edge_targets)  # shape: [CE]
        ...
        distance_embedding = tf.nn.embedding_lookup(self._distance_embedding, truncated_distances)  # shape: [CE, 1]
        ...
        edge_type_logits = self._edge_type_selector(edge_candidate_representation, training=training)  # shape: [CE, ET]
    ...
    def compute_node_type_selection_loss(
        ...
        per_node_decision_logprobs = tf.nn.log_softmax(node_type_logits, axis=-1)  # Shape: [NTP, NT + 1]
        ...
        no_node_decision_correct = tf.math.equal(per_node_decision_num_correct_choices, 0.0)  # Shape [NTP]
        per_correct_no_node_decision_neglogprob = -(
            per_node_decision_logprobs[:, -1] * tf.cast(tf.squeeze(no_node_decision_correct), tf.float32)
        )  # Shape [NTP]
        ...
        total_node_type_loss = tf.reduce_sum(per_correct_node_decision_normalised_neglogprob) + tf.reduce_sum(
            per_correct_no_node_decision_neglogprob
        )
        node_type_loss = safe_divide_loss(total_node_type_loss, tf.shape(node_type_multihot_labels)[0])
    ...
    def compute_edge_candidate_selection_loss(
        ...
        edge_candidate_to_graph_map = tf.gather(node_to_graph_map, candidate_edge_targets)  # Shape: [CE]
        ...
        per_graph_num_correct_edge_choices = tf.maximum(per_graph_num_correct_edge_choices, 1)  # Shape: [PG]
        ...
        edge_loss = safe_divide_loss(tf.reduce_sum(per_correct_edge_neglogprob), num_graphs_in_batch)
    ...
    def compute_edge_type_selection_loss(
        ...
        masked_edge_type_logits = edge_type_logits_for_correct_edges - scaled_edge_mask  # Shape: [CCE, ET]
        ...
        edge_type_loss = safe_divide_loss(tf.reduce_sum(edge_type_loss), tf.shape(edge_type_loss)[0])
    ...
    def _add_atom_or_motif(
        ...
        node_type: str,
        logprob: float,
        choice_info: Optional[Union[MoleculeGenerationAtomChoiceInfo, MoleculeGenerationAttachmentPointChoiceInfo]],
    ...
    def decode(
        ...
        initial_molecules = [Chem.Mol() if initMol is None else initMol for initMol in initial_molecules]
        ...
        for graph_repr, init_mol, mol_id in zip(graph_representations, initial_molecules, mol_ids):
            ...
            for atom in init_mol.GetAtoms():
                if atom.GetAtomicNum() == 0:
                    ...
                    if len(bonds) > 1:
                        ...
                        raise ValueError(f"Scaffold {scaffold} contains a [*] atom with at least two bonds.")
            ...
            for bond in init_mol.GetBonds():
                ...
                adjacency_lists[bond_type_idx].append((bond.GetEndAtomIdx(), bond.GetBeginAtomIdx()))
        ...
        while num_steps < max_num_steps:
            ...
            for decoder_state, (node_type_picks, node_type_logprobs) in zip(require_atom_states, node_pick_results):
            ...
            for decoder_state, (bond_picks, edge_choice_info) in zip(require_bond_states, bond_pick_results):
                if len(bond_picks) == 0:
                    ...
                    if bond_pick is None:
                        ...
                        new_decoder_states.append(
                            MoLeRDecoderState.new_with_added_bond(
                                old_state=decoder_state,
                                target_atom_idx=int(picked_bond_target),  # Go from np.int32 to pyInt
    ...
    def _decoder_pick_new_atom_types(
        ...
        *,
        ...
        def add_state_to_atom_choice_batch(batch: Dict[str, Any], decoder_state: MoLeRDecoderState):
            batch["prior_focus_atoms"].append(decoder_state.prior_focus_atom + batch["nodes_in_batch"])
        ...
        atom_type_pick_generator = (
            self._pick_new_atom_types_for_batch(batch, num_samples, sampling_mode) for batch, _ in batch_generator
        )
    ...
    def _pick_new_atom_types_for_batch(
        ...
        atom_type_logprobs = tf.nn.log_softmax(node_type_logits[:, 1:], axis=1).numpy()  # Shape [G, NT]
    ...
    def _decoder_pick_first_atom_types(
        ...
        *,
        ...
        molecule_representations = np.stack([state.molecule_representation for state in decoder_states])

        first_node_type_logits = self.pick_first_node_type(
            input_molecule_representations=molecule_representations, training=False
        )  # Shape [G, NT + 1]

        first_atom_type_logprobs = tf.nn.log_softmax(first_node_type_logits[:, 1:], axis=1).numpy()  # Shape [G, NT]

        first_atom_type_pick_results: List[Tuple[List[Tuple[Optional[str], float]], np.ndarray]] = []
        ...
        for state_first_atom_type_logprobs in first_atom_type_logprobs:
            ...
            for picked_atom_type_idx in picked_atom_type_indices:
                ...
                this_state_results.append((self._index_to_node_type_map[picked_atom_type_idx], pick_logprob))

            first_atom_type_pick_results.append((this_state_results, state_first_atom_type_logprobs))
    ...
    def _decoder_pick_new_bond_types(
        ...
        *,
        ...
        store_generation_traces: bool = False,
        num_samples: int = 1,
    ) -> Iterable[Tuple[List[Tuple[Optional[Tuple[int, int]], float]], Optional[MoleculeGenerationEdgeChoiceInfo],]]:
        """
        ...
        def add_state_to_edge_batch(batch: Dict[str, Any], decoder_state: MoLeRDecoderState):
            ...
            batch["candidate_edge_features"].append(decoder_state.compute_bond_candidate_features(candidate_targets))
    ...
    def _pick_edges_for_batch(
        ...
        edge_candidate_logits, edge_type_logits = self.pick_edge(
            ...
            candidate_edge_features=np.concatenate(batch["candidate_edge_features"], axis=0).astype(np.float32),
            training=False,
        )  # Shape [VE + PG, 1], [VE, ET]
        ...
        for state_idx, (decoder_state, decoder_state_num_edge_candidates) in enumerate(
            ...
            decoder_state_no_edge_logit = edge_candidate_logits[num_total_edge_candidates + state_idx]

            decoder_state_edge_cand_logprobs = tf.nn.log_softmax(
                tf.concat([decoder_state_edge_candidate_logits, [decoder_state_no_edge_logit]], axis=0)
            )
                ...
                for edge_idx, (target, score, logprob) in enumerate(
                    ...
                    molecule_generation_edge_candidate_info.append(
                        MoleculeGenerationEdgeCandidateInfo(
                            ...
                            logprob=logprob,
                            correct=None,
                            type_idx_to_logprobs=tf.nn.log_softmax(masked_candidate_edge_type_logits[edge_idx, :]),
                        )
                    )
            ...
            for picked_edge_cand_idx in picked_edge_cand_indices:
                ...
                if picked_edge_cand_idx == len(decoder_state_edge_cand_logprobs) - 1:
                    ...
                    edge_type_mask = batch_candidate_edge_type_masks[edge_candidate_offset + picked_edge_cand_idx]
                    ...
                    for picked_edge_type in picked_edge_types:
                        picked_edge_logprob = picked_cand_logprob + cand_edge_type_logprobs[picked_edge_type]
                        this_state_results.append(((picked_edge_partner, picked_edge_type), picked_edge_logprob))
    ...
    def _decoder_pick_attachment_points(
        ...
        *,
        ...
        def add_state_to_attachment_point_choice_batch(batch: Dict[str, Any], decoder_state: MoLeRDecoderState):
    ...
    def _pick_attachment_points_for_batch(
        ...
        focus_atoms = np.array([attachment_points[0] for attachment_points in batch["candidate_attachment_points"]])
        ...
        attachment_point_to_graph_map = tf.gather(batch["node_to_graph_map"], candidate_attachment_points)
        ...
        for logit, graph_id in zip(attachment_point_selection_logits, attachment_point_to_graph_map):
        ...
        for old_decoder_state, attachment_point_logits in zip(decoder_states, logits_by_graph):
            ...
            for attachment_point_pick_idx in picked_att_point_indices:
                attachment_point_pick = old_decoder_state.candidate_attachment_points[attachment_point_pick_idx]
