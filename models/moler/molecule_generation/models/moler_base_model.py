...
class MoLeRBaseModel(GraphTaskModel):
    ...
    def get_default_hyperparameters(cls, mp_style: Optional[str] = None) -> Dict[str, Any]:
        ...
        decoder_hypers = {cls.decoder_prefix() + k: v for k, v in MoLeRDecoder.get_default_params(mp_style).items()}
    ...
    def __init__(self, params: Dict[str, Any], dataset: TraceDataset, **kwargs):
        ...
        self._train_step_counter = tf.Variable(initial_value=0, trainable=False, dtype=tf.int32, name="training_step")
        ...
        if self.uses_categorical_features:
            if "categorical_features_embedding_dim" in self._params:
                ...
                self._params["categorical_features_embedding_dim"] = self._node_categorical_num_classes
                self._node_categorical_features_embedding = np.eye(self._node_categorical_num_classes, dtype=np.float32)
        ...
        with tf.name_scope("decoder"):
            ...
            self._decoder_layer.build(
                MoLeRDecoderInput(
                    ...
                    graphs_requiring_node_choices=input_shapes["partial_graphs_requiring_node_choices"],
    ...
    def _compute_decoder_loss_and_metrics(
        ...
        total_loss = (
            self._params["node_classification_loss_weight"] * decoder_metrics.node_classification_loss
            + self._params["first_node_classification_loss_weight"] * decoder_metrics.first_node_classification_loss
        ...
        if self.uses_motifs:
            total_loss += (
                self._params["attachment_point_selection_weight"] * decoder_metrics.attachment_point_selection_loss
            )

        return total_loss, decoder_metrics

    def _get_decoder_output(self, *, batch_features, molecule_representations, training) -> MoLeRDecoderOutput:
        ...
        decoder_output = self._decoder_layer(
            MoLeRDecoderInput(
                ...
                graphs_requiring_node_choices=batch_features["partial_graphs_requiring_node_choices"],
    ...
    def _get_graph_generation_losses(self, task_results: List[Any]) -> List[Tuple[str, str]]:
        ...
        average_node_classification_loss = self._dict_average(task_results, "node_classification_loss")
        average_first_node_classification_loss = self._dict_average(task_results, "first_node_classification_loss")
