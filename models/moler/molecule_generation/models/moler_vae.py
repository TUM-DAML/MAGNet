...
from models.moler.molecule_generation.layers.moler_decoder import MoLeRDecoderMetrics
from models.moler.molecule_generation.models.moler_base_model import MoLeRBaseModel
from models.moler.molecule_generation.utils.property_models import (
...
class MoLeRVae(MoLeRBaseModel):
    ...
    def compute_latent_molecule_representations(
        ...
        weighted_avg_graph_repr = self._weighted_avg_of_nodes_to_graph_repr(node_to_graph_repr_input, training=training)
        weighted_sum_graph_repr = self._weighted_sum_of_nodes_to_graph_repr(node_to_graph_repr_input, training=training)
        input_graph_encodings = tf.concat([weighted_avg_graph_repr, weighted_sum_graph_repr], axis=-1)

        graph_mean_and_log_variance = self._mean_and_var_mlp(input_graph_encodings, training=training)
    ...
    def compute_task_output(
        ...
        (
            ...
            partial_graph_to_original_graph_map=batch_features["partial_graph_to_original_graph_map"],
            training=training,
        )
    ...
    def compute_vae_metrics(
        ...
        normalised_kl_loss = kl_divergence_loss / tf.cast(batch_features["num_graphs_in_batch"], tf.float32)
    ...
    def compute_epoch_metrics(self, task_results: List[Any]) -> Tuple[float, str]:
        ...
        for prop_name, prop_results in prop_to_task_results.items():
            prop_num_samples = np.sum(batch_prop_result["num_samples"] for batch_prop_result in prop_results)
            ...
            property_epoch_metric += self._property_predictor_params[prop_name]["loss_weight_factor"] * prop_metric
