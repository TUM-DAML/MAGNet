...
from models.moler.molecule_generation.dataset.trace_dataset import TraceDataset
from models.moler.molecule_generation.layers.cgvae_decoder import (
    CGVAEDecoder,
    CGVAEDecoderInput,
)
from models.moler.molecule_generation.layers.graph_property_predictor import (
    GraphPropertyPredictor,
    GraphPropertyPredictorInput,
)
from models.moler.molecule_generation.utils.epoch_metrics_logger import (
    EpochMetricsLogger,
)
from models.moler.molecule_generation.utils.training_utils import (
    get_class_balancing_weights,
)
...
class CGVAE(GraphTaskModel):
    ...
    def get_default_hyperparameters(cls, mp_style: Optional[str] = None) -> Dict[str, Any]:
        ...
        decoder_hypers = {cls.decoder_prefix() + k: v for k, v in CGVAEDecoder.get_default_params(mp_style).items()}
    ...
    def __init__(self, params: Dict[str, Any], dataset: TraceDataset, **kwargs):
        ...
        self._feature_extractors: List[AtomFeatureExtractor] = dataset.metadata.get("feature_extractors", [])
        ...
        for prop_name in self._graph_property_params.keys():
            self._graph_property_metadata[f"{prop_name}_stddev"] = dataset.metadata.get(f"{prop_name}_stddev")
        ...
        property_predictor_default_hypers = {
            k[n:]: v for k, v in self._params.items() if k.startswith(self.property_predictor_prefix())
        }
    ...
    def build(self, input_shapes: Dict[str, Any]):
        ...
        if self._params["use_intermediate_gnn_results"]:
            ...
            final_node_representation_spec: Tuple[tf.TensorSpec, ...] = (
                tf.TensorSpec(shape=(None, encoder_gnn_dim), dtype=tf.float32),  # the actual final reprs
                tuple(
                    tf.TensorSpec(shape=(None, encoder_gnn_dim), dtype=tf.float32)  # the per per-layer reprs
        ...
        encoded_node_representation_shape = tf.TensorShape(dims=(None, final_node_representation_dim))
        ...
        with tf.name_scope("decoder"):
            partial_adjacency_lists: Tuple[tf.TensorShape, ...] = tuple(
                input_shapes[f"partial_adjacency_list_{edge_type_idx}"] for edge_type_idx in range(self._num_edge_types)
            )
    ...
    def compute_node_mean_and_logvariance(
        ...
        if self._params["use_intermediate_gnn_results"]:
            # We get the initial GNN input (after projection) + results for all layers:
            node_representations = tf.concat(node_representations[1], axis=-1)  # Shape [V, ED*(num_layers+1)]
    ...
    def generate_samples(
        ...
        if self._sample_strategy == "passthrough":
            ...
            return self._generate_per_graph_samples(node_mean, node_log_variance, partial_node_to_original_node_map)
    ...
    def _generate_per_graph_samples(self, mean: tf.Tensor, logvariance: tf.Tensor, node_map: tf.Tensor) -> tf.Tensor:
    ...
    def classify_nodes(self, node_features: tf.Tensor) -> List[str]:
        ...
        """
        node_classification_logits = self._node_to_label_layer(node_features, training=False)  # shape: [V, NT]
    ...
    def get_node_samples(
        ...
        node_mean, node_log_variance = self.compute_node_mean_and_logvariance(final_node_representations, training)
        ...
        node_samples = self.generate_samples(node_mean, node_log_variance, tf.range(tf.shape(node_mean)[0]))
    ...
    def compute_task_output(
        ...
        partial_adjacency_lists: Tuple[tf.Tensor, ...] = tuple(
            batch_features[f"partial_adjacency_list_{edge_type_idx}"] for edge_type_idx in range(self._num_edge_types)
        ...
        for prop_name, property_predictor in self._property_predictors.items():
            ...
            property_prediction_results[prop_name] = property_predictor(prop_prediction_input, training=training)
    ...
    def compute_vae_metrics(
        ...
        kl_divergence = tf.reduce_sum(kl_divergence_summand) / (2 * num_graphs_in_batch)  # Shape: []
    ...
    def compute_epoch_metrics(self, task_results: List[Any]) -> Tuple[float, str]:
        ...
        for prop_name, prop_results in prop_to_task_results.items():
            prop_metric, prop_metric_description = self._property_predictors[prop_name].compute_epoch_metrics(
                prop_results
            )
            property_epoch_metric += self._graph_property_params[prop_name]["loss_weight_factor"] * prop_metric
