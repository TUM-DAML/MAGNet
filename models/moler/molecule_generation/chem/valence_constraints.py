...
from baselines.moler.molecule_generation.chem.rdkit_helpers import get_true_symbol, get_charge_from_symbol
...
def constrain_edge_types_based_on_valence(
    ...
    node_idx_to_valid_incoming_edge_mask = _calculate_valid_edge_mask(node_idx_to_open_slots, num_edge_types)
    return node_idx_to_valid_incoming_edge_mask[candidate_target_nodes]


def _calculate_valid_edge_mask(node_idx_to_open_slots: np.ndarray, num_edge_types: int) -> np.ndarray:
    ...
    node_idx_to_max_valence_to_remove = np.minimum(node_idx_to_open_slots, max_valence_for_node_types)

    edge_type_mask = 1 - np.triu(np.ones(shape=(num_edge_types + 1, num_edge_types), dtype=np.float64))
...
def _calculate_max_valency(node_types: List[str]) -> np.ndarray:
    ...
    """
    node_idx_to_max_valency_map = np.array([_max_valence_for_single_node(node_type) for node_type in node_types])
