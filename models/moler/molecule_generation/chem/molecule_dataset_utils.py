...
from rdkit.Chem.Descriptors import BertzCT, ExactMolWt
...
from models.moler.molecule_generation.chem.rdkit_helpers import get_atom_symbol
from models.moler.molecule_generation.utils.sequential_worker_pool import (
    get_worker_pool,
)
...
def compute_smiles_dataset_metadata(
    ...
    if not featurisers_provided:
        ...
        uninitialised_featurisers = [
            featuriser for featuriser in atom_feature_extractors if not featuriser.metadata_initialised
        ]
...
def featurise_mol_data(
    ...
    for datapoint in mol_data:
        try:
            ...
            print(f"Skipping datapoint {datapoint['SMILES']}, cannot featurise with current metadata.")
            continue
...
def featurise_smiles_datapoints(
    *,
    ...
    if motif_vocabulary is not None and motif_extraction_settings is not None:
        logger.warning("Motif vocabulary was already provided, so extraction settings will be ignored.")
...
def featurise_atoms(
    ...
    if motif_vocabulary is not None:
        atom_type_feature_extractor = next(
            featuriser for featuriser in atom_feature_extractors if isinstance(featuriser, AtomTypeFeatureExtractor)
        )
    ...
    for atom_id, atom in enumerate(mol.GetAtoms()):
        ...
        atom_features = [atom_featuriser.featurise(atom) for atom_featuriser in atom_feature_extractors]
...
def molecule_to_graph(
    ...
    if _need_kekulize(mol):
        ...
        if _need_kekulize(mol):
            raise ValueError(
                f"Given molecule cannot be Kekulized successfully. " f"Molecule has smiles string:\n{MolToSmiles(mol)}"
            )
        if mol is None:
            return None
    ...
    graph["node_features"] = [atom_features.tolist() for atom_features in node_features.real_valued_features]
...
def get_substructure_match(mol: Mol, scaffold_smarts: str) -> Tuple[int]:
    ...
    if len(substructure_matches) == 0:
        ...
        if len(substructure_matches) == 0:
            raise ValueError(f"No substructure match found for {scaffold_smarts} in molecule {MolToSmiles(mol)}")
...
def compute_sa_score(mol: Chem.Mol, sascorer_path: Optional[str] = None) -> float:
    ...
    if __compute_sascore is None:
        ...
        if sascorer_path is None:
            sascorer_path = os.path.join(os.path.normpath(RDConfig.RDContribDir), "SA_Score", "sascorer.py")
