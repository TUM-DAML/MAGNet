...
from baselines.moler.molecule_generation.dataset.trace_dataset import TraceDataset, TraceSample
from baselines.moler.molecule_generation.utils.sharded_data_reader import ShardedDataReader
...
class JSONLAbstractTraceDataset(TraceDataset):
    ...
    def load_data(self, path: RichPath, folds_to_load: Optional[Set[DataFold]] = None) -> None:
        ...
        self._atom_type_featuriser = next(
            featuriser for featuriser in self._metadata["feature_extractors"] if featuriser.name == "AtomType"
        )
        ...
        def find_fold_files(directory: RichPath, fold_prefix: str) -> List[RichPath]:
            ...
            return [
                path for path in directory.get_filtered_files_in_dir(f"**/{fold_prefix}*.pkl.gz") if path.is_file()
            ]
    ...
    def _graph_iterator(self, data_fold: DataFold, no_parallelism: bool = False) -> ShardedDataReader[TraceSample]:
