...
from baselines.moler.molecule_generation.utils.moler_decoding_utils import DecoderSamplingMode
from baselines.moler.molecule_generation.utils.model_utils import get_model_class, get_model_parameters
...
class ModelWrapper(ContextManager):
    ...
    def _get_model_file(cls, dir: Pathlike) -> pathlib.Path:
        ...
        if len(candidates) != 1:
            raise ValueError(f"There must be exactly one *.pkl file. Found the following: {candidates}.")
        else:
            return candidates[0]

    def __enter__(self):
        from baselines.moler.molecule_generation.utils.moler_inference_server import (
...
class VaeWrapper(ModelWrapper):
    ...
    def encode(
        ...
        return self._inference_server.encode(smiles_list, include_log_variances=include_log_variances)
    ...
    def decode(
        ...
        if scaffolds is not None:
            scaffolds = [Chem.MolFromSmiles(scaffold) if scaffold is not None else None for scaffold in scaffolds]

        return [
            (smiles_str, gen_trace)
            for smiles_str, _, gen_trace in self._inference_server.decode(
