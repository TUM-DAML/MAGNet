...
from baselines.moler.molecule_generation.dataset.jsonl_abstract_trace_dataset import JSONLAbstractTraceDataset
from baselines.moler.molecule_generation.dataset.jsonl_cgvae_trace_dataset import JSONLCGVAETraceDataset
from baselines.moler.molecule_generation.dataset.jsonl_moler_trace_dataset import JSONLMoLeRTraceDataset
from baselines.moler.molecule_generation.models.cgvae import CGVAE
...
from baselines.moler.molecule_generation.utils.cli_utils import setup_logging, supress_tensorflow_warnings
...
def run_from_args(args: argparse.Namespace) -> Tuple[str, str, str]:
    ...
    if not isinstance(loaded_model_dataset[0], JSONLAbstractTraceDataset):
        raise ValueError(
            f"This training script can only work with TraceDatasets, " f"but got {loaded_model_dataset[0]}!"
        )
    ...
    if not isinstance(loaded_model_dataset[1], (CGVAE, MoLeRVae, MoLeRGenerator)):
        raise ValueError(
            f"This training script can only work with CGVAE/MoLeR, " f"but got {loaded_model_dataset[1]}!"
        )
...
def train(
    ...
    with dataset.get_context_managed_tf_dataset(training_utils.DataFold.TRAIN) as train_data:
        ...
        with dataset.get_context_managed_tf_dataset(training_utils.DataFold.VALIDATION) as valid_data:
