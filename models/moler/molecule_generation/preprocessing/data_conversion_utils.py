...
from models.moler.molecule_generation.chem.molecule_dataset_utils import featurise_atoms
...
from models.moler.molecule_generation.dataset.trace_sample import TraceSample
...
def __inner_convert_graph_sample_to_graph_trace(
    ...
    for trace_step in generation_trace:
        ...
        constrained_distances = np.array(trace_step.distance_to_target, dtype=np.float32)[edge_constraint_mask].reshape(
            -1, 1
        )
    ...
    for motif_type, atom_annotations in datum.get("motifs", []):
        motifs.append(
            MotifAnnotation(
                ...
                atoms=[
                    MotifAtomAnnotation(atom_id, symmetry_class_id) for atom_id, symmetry_class_id in atom_annotations
                ],
            )
        )
