...
from baselines.moler.molecule_generation.chem.rdkit_helpers import compute_canonical_atom_order
...
class GenerationOrder:
    def __init__(self, mol: Union[Mol, RWMol], enclosing_motif_id: List[Optional[int]]):
        ...
        self._node_id_to_position = {node_id: position for position, node_id in enumerate(self._atom_order)}
...
class LoopClosingOrder(GenerationOrder):
    ...
    def get_valid_next_node_choices(self, frontier: List[int], visited: List[int]) -> List[int]:
        ...
        if max_overlap_rings:
            next_node_choices = [
                node_id for node_id in frontier if any(node_id in ring for ring in max_overlap_rings)
            ]
        else:
