...
from models.moler.molecule_generation.chem.valence_constraints import (
    _max_valence_for_single_node,
)
from models.moler.molecule_generation.preprocessing.generation_order import (
    BFSOrder,
    GenerationOrder,
)
from models.moler.molecule_generation.preprocessing.graph_sample import (
    AdjacencyList,
    Edge,
    GraphSample,
    GraphTraceSample,
...
def graph_sample_to_MoLeR_trace(
    ...
    re_indexed_adjacency_list = reindex_adjacency_list(graph_sample.adjacency_list, exploration_order)
    ...
    for focus_node_idx, valid_next_node_choices in enumerate(all_valid_next_node_choices):
        ...
        if current_motif_id is None:
            ...
            for node_idx in nodes_to_add:
                ...
                for edge in node_idx_to_neighbours[node_idx]:
                    ...
                    if enclosing_motif_id[edge.target] == current_motif_id and edge.target < node_idx:
                        add_edge(edge)
        ...
        add_generation_step(
            ...
            step_correct_node_type_choices=[
                node_types_for_next_type_prediction[node_idx] for node_idx in valid_next_node_choices
            ],
            step_correct_edge_choices=[],
            step_valid_edge_choices=edges_to_open_nodes,
            step_correct_attachment_point_choice=None,
    ...
    if graph_sample.node_categorical_features is not None:
        node_categorical_features = reindex_list(graph_sample.node_categorical_features, exploration_order)
    else:
...
def reindex_motifs(motifs: List[MotifAnnotation], exploration_order: List[int]) -> List[MotifAnnotation]:
    ...
    for motif in motifs:
        ...
        for atom in motif.atoms:
            reindexed_atoms.append(
                MotifAtomAnnotation(atom_id=idx_map[atom.atom_id], symmetry_class_id=atom.symmetry_class_id)
            )
...
def reindex_adjacency_list(adjacency_list: List[Edge], exploration_order: List[int]) -> List[Edge]:
    ...
    """
    idx_map = _get_old_id_to_new_id_mapping(exploration_order)
    return [Edge(source=idx_map[e.source], target=idx_map[e.target], type=e.type) for e in adjacency_list]
...
def calculate_dist_from_focus_to_valid_target(
    ...
    valid_target_set = {edge.target for edge in valid_edges if node_states[edge.target] in valid_target_node_states}
...
def convert_lists_to_graph_trace_sample(
    ...
    partial_graphs = [
        ...
        for partial_node_list, partial_adj_list, mol in zip(partial_node_lists, partial_adjacency_lists, molecule_trace)
    ]
    ...
    for partial_node_list, partial_adj_list, mol in zip(partial_node_lists, partial_adjacency_lists, molecule_trace):
        ...
        assert partial_node_list == list(range(num_nodes)), "Nodes belonging to a partial graph are not consecutive."
...
def get_node_edges(
    ...
    locked_edges = [typed_edge for typed_edge in candidate_edges if node_states[typed_edge.target] is NodeState.LOCKED]
    return locked_edges
...
def get_open_edges(focus_node_idx: int, node_states: Dict[int, NodeState]) -> EdgeCollection:
    ...
    unlocked_edges = [Edge(source=focus_node_idx, target=target, type=-1) for target in locked_nodes]
