...
from typing import Any, DefaultDict, Iterable, List, NamedTuple, Optional, Tuple, Union
...
from models.moler.molecule_generation.chem.molecule_dataset_utils import (
    BOND_DICT,
    featurise_atoms,
)
...
from models.moler.molecule_generation.preprocessing.moler_generation_trace import (
    Edge,
...
class MoLeRDecoderState(object):
    ...
    def new_with_added_bond(
        ...
        if old_state._focus_atom is None:
            raise ValueError("New bonds can only be added when the decoder is focused on creating bonds for an atom!")
        new_mol = Chem.RWMol(old_state._molecule)
        new_mol.AddBond(old_state._focus_atom, target_atom_idx, EDGE_TYPE_IDX_TO_BOND_TYPE[bond_type_idx])
    ...
    def new_with_added_motif(
        ...
        new_motifs.append(
            MotifAnnotation(
                ...
                atoms=[MotifAtomAnnotation(atom_id=atom_id, symmetry_class_id=-1) for atom_id in motif_nodes],
            )
        )
    ...
    def new_with_focus_marked_as_visited(
        ...
        if len(old_state._atoms_to_visit) > 0:
            new_atoms_to_visit = list(old_state._atoms_to_visit)
            new_focus_atom: Optional[int] = new_atoms_to_visit.pop(0)  # BFS exploration; .pop() would give DFS
        else:
    ...
    def get_bond_candidate_targets(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        candidate_targets = {idx for idx in self._visited_atoms if self._num_free_bond_slots[idx] != 0}
    ...
    def compute_bond_candidate_features(self, candidate_targets) -> np.ndarray:
        ...
        return np.concatenate([np.expand_dims(distance_features, axis=-1), topology_features], axis=1)
...
def restrict_to_beam_size_per_mol(
    ...
    for per_mol_decoder_states in grouped_by_mol.values():
        restricted_decoder_states.extend(heapq.nlargest(beam_size, per_mol_decoder_states, key=lambda s: s.logprob))
