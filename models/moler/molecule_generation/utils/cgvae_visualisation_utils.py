...
from models.moler.molecule_generation.models.cgvae import (
    CGVAEOutput,
...
class GraphGenerationVisualiser(ABC):
    ...
    def visualise_from_smiles(self, smiles: str):
        ...
        self.render_atom_data(
            [
                AtomPredictionInformation(
                    ...
                    type_idx_to_prob=tf.nn.softmax(predictions.node_classification_logits[node_idx, :]).numpy(),
                )
        ...
        for step, focus_node_idx in enumerate(batch_features["focus_nodes"].numpy()):
            ...
            edge_choice_scores = [
                predictions.edge_logits[edge_choice_index].numpy() for edge_choice_index in edge_choice_indices
            ]
            # The special "no more edges" choices are appended to the end, so get that one:
            edge_choice_scores.append(predictions.edge_logits[total_num_valid_edge_choices + step].numpy())
    ...
    def visualise_from_samples(self, node_representations: np.ndarray):
        ...
        for prop_name in self.supported_property_names:
            property_data[prop_name] = PropertyPredictionInformation(
                prediction=self.vae._property_predictors[prop_name](prop_prediction_input, training=False).numpy(),
                ground_truth=None,
            )
        self.render_property_data(property_data)

        # Second, run and render the atom type predictions:
        node_classification_logits = self.vae._node_to_label_layer(node_representations, training=False)
        self.render_atom_data(
            [
                AtomPredictionInformation(
                    node_idx=node_idx,
