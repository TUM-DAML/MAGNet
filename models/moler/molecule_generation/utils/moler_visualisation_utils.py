...
from models.moler.molecule_generation.dataset.trace_dataset import DataFold
from models.moler.molecule_generation.layers.moler_decoder import (
...
from models.moler.molecule_generation.models.moler_vae import MoLeRVaeOutput
from models.moler.molecule_generation.utils.model_utils import (
    load_vae_model_and_dataset,
)
from models.moler.molecule_generation.wrapper import VaeWrapper
...
class GraphGenerationVisualiser(ABC):
    def __init__(self, model_dir: str):
        ...
        self.dataset.params["trace_element_keep_prob"] = 1.0  # Make sure that we keep all trace steps
    ...
    @abstractmethod
    def render_molecule_gen_edge_step(self, step: int, step_info: MoleculeGenerationEdgeChoiceInfo) -> None:
        pass
    ...
    def get_atom_and_motif_types_to_render(
        ...
        for type_idx in range(1, num_types):
            ...
            if prob >= prob_threshold or is_correct:
                types_to_render.append((type_idx, prob, self.dataset._node_type_index_to_string[type_idx]))
    ...
    def visualise_from_smiles(self, smiles: str):
        ...
        for choice_idx, src_node_idx in enumerate(batch_features["valid_edge_choices"][:, 0].numpy()):
        ...
        steps_requiring_node_choices = batch_features["partial_graphs_requiring_node_choices"].numpy()
        ...
        for node_idx in valid_attachment_point_choices:
            ...
            graph_to_valid_attachment_point_choices[graph_idx].append(partial_node_to_orig_node_id[node_idx])

        for node_idx in correct_attachment_point_choices:
            node_idx = valid_attachment_point_choices[node_idx]

            graph_idx = node_to_partial_graph[node_idx]
            graph_to_correct_attachment_point_choice[graph_idx] = partial_node_to_orig_node_id[node_idx]

        # First, render the initial atom choice:
        first_node_choice_one_hot_labels = batch_labels["correct_first_node_type_choices"][0].numpy()
        ...
        for step, focus_node_idx in enumerate(batch_features["focus_nodes"].numpy()):
            ...
            if step in graph_to_valid_attachment_point_choices:
                indices = list(np.where(node_to_partial_graph[valid_attachment_point_choices] == step)[0])
                ...
                num_nodes_in_previous_graph = num_nodes_per_partial_graph[step - 1] if step > 0 else 0

                added_motif_nodes = list(range(num_nodes_in_previous_graph, num_nodes_in_current_graph))
                ...
                for i, edge_choice_index in enumerate(edge_choice_indices):
                    ...
                    type_logprobs = tf.nn.log_softmax(predictions.edge_type_logits[edge_choice_index, :])
            ...
            if step in steps_requiring_node_choices:
                ...
                self.render_atom_data(
                    MoleculeGenerationAtomChoiceInfo(
                        ...
                        type_idx_to_prob=tf.nn.softmax(predictions.node_type_logits[node_choice_idx, :]).numpy(),
                    ),
    ...
    def visualise_from_samples(self, molecule_representation: np.ndarray):
        ...
        for prop_name in self.supported_property_names:
            property_data[prop_name] = PropertyPredictionInformation(
                prediction=self.vae._property_predictors[prop_name]([molecule_representation], training=False).numpy(),
                ground_truth=None,
            )
        ...
        for step_info in decoder_states.generation_steps:
            if isinstance(step_info, MoleculeGenerationAttachmentPointChoiceInfo):
                ...
                self.render_atom_data(
                    ...
                    choice_descr=("next addition to partial molecule" if first_step_done else "initial starting point"),
                )
            else:
