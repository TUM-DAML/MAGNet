...
class Ray:
    ...
    def construct_ray(
        ...
        if adjacency_lists is None:
            adjacency_lists = [np.zeros(shape=(0, 2), dtype=np.int32) for _ in range(num_edge_types)]
        ...
        if symmetrize_adjacency_lists:
            for i, adjacency_list in enumerate(adjacency_lists):
                ...
                symmetrised_adjacency_list = np.empty(shape=(2 * len(adjacency_list), 2), dtype=adjacency_list.dtype)
        ...
        if add_self_loop_edges:
            adjacency_lists.append(np.repeat(np.arange(num_nodes, dtype=np.int32), 2).reshape(-1, 2))
        ...
        exploration_queue = [
            node_idx for node_idx, node_state in node_states.items() if node_state == NodeState.DISCOVERED
        ]
        return Ray(
            logprob=0.0,
            idx=idx,
    ...
    def add_edge(self, edge: np.ndarray, edge_type: int):
        ...
        self._molecule.AddBond(edge_source.item(), edge_target.item(), self._edge_type_idx_to_rdkit_type[edge_type])
...
def extend_beam(extension_choices: List[RayExtension], beam: List[Ray]) -> List[Ray]:
    ...
    for i, extension_choice in enumerate(extension_choices):
        ...
        if new_beam[i].generation_trace is not None and extension_choice.generation_step_info is not None:
