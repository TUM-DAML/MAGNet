...
from models.moler.molecule_generation.chem.motif_utils import MotifVocabulary
...
def batch_decoder_states(
    *,
    ...
    for decoder_state in decoder_states:
        node_features, node_categorical_features = decoder_state.get_node_features(atom_featurisers, motif_vocabulary)
        mol_num_nodes = node_features.shape[0]

        if mol_num_nodes >= max_nodes_per_batch:
            logger.warning(f"Single graph of {mol_num_nodes} nodes exceeds max_nodes_per_batch {max_nodes_per_batch}")
...
def _finalise_batch(batch: Dict[str, Any], uses_categorical_features: bool) -> None:
    ...
    if uses_categorical_features:
        batch["node_categorical_features"] = np.concatenate(batch["node_categorical_features"], axis=0)
    ...
    batch["adjacency_lists"].append(np.repeat(np.arange(batch["nodes_in_batch"], dtype=np.int32), 2).reshape(-1, 2))
