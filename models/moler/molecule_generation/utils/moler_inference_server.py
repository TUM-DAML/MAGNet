...
from baselines.moler.molecule_generation.dataset.in_memory_trace_dataset import (
    DataFold,
    InMemoryTraceDataset,
)
from baselines.moler.molecule_generation.models.moler_generator import MoLeRGenerator
from baselines.moler.molecule_generation.models.moler_vae import MoLeRVae
from baselines.moler.molecule_generation.preprocessing.data_conversion_utils import remove_non_max_frags
...
def _encode_from_smiles(
    ...
    for smiles_str in smiles_list:
        try:
            _, trace_sample = dataset.transform_smiles_to_sample(smiles_str, include_generation_trace=False)
    ...
    result = []
    for batch_features, _ in dataset.get_tensorflow_dataset(data_fold=DataFold.TEST, use_worker_threads=False):
        final_node_representations = model.compute_final_node_representations(batch_features, training=False)
...
def _decode_from_latents(
    ...
    # decoder_states[0]._motifs should have all the information we need
...
class MoLeRInferenceServer(object):
    ...
    def cleanup_workers(self, ignore_failures: bool = False):
        ...
        if not ignore_failures and len(issues_req_ids) > 0:
            raise Exception(f"Failed to clean up all worker threads! Missing worker IDs: {issues_req_ids}")
