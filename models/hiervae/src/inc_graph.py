...
from baselines.hiervae.src.chemutils import *
from baselines.hiervae.src.mol_graph import MolGraph


class IncBase(object):
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=200, max_edges=200, max_nb=12):
        self.max_nb = max_nb
...
class IncGraph(IncBase):
    ...
    def add_mol(self, batch_idx, smiles, inter_label, nth_child):
        ...
        for (
            atom
        ) in emol.GetAtoms():  # atoms must be inserted in order given by emol.GetAtoms() (for rings assembly)
