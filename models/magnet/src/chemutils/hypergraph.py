...
from models.magnet.src.chemutils.constants import ATOM_LIST
from models.magnet.src.chemutils.rdkit_helpers import compute_fingerprint
    ...
    def prepare_batch_output(self, mfeat_shape=(512,)):
        ...
        for key, _ in self.id_to_hash.items():
            ...
            # cyclic junctions
                ...
                is_junc_atom = (~np.array([is_atom_cyclic_junction(a) for a in motif.GetAtoms()])).astype(int)[s_idx]
        ...
        def map_hash_to_feat(hashes):
            return [
                np.full(mfeat_shape, fill_value=-1) if h == -1 else feats_per_motif[self.hash_to_id[h]] for h in hashes
            ]
