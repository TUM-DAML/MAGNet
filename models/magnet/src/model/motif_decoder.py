...
from baselines.magnet.src.chemutils.hypergraph import (
    is_all_cyclic,
    is_atom_cyclic_junction,
    is_cyclic_junction,
)
from baselines.magnet.src.chemutils.motif_helpers import atom_multiset_to_counts
from baselines.magnet.src.chemutils.rdkit_helpers import (
    MolFromGraph,
    compute_fingerprint,
    get_atom_charges,
    get_max_valency,
...
from baselines.magnet.src.data.utils import get_atom_multiplicity_from_unsorted, get_atom_positional
from baselines.magnet.src.model.transformer_utils import transformer_forward
from baselines.magnet.src.utils import calculate_balanced_acc, calculate_class_weights
...
class MotifBondDecoder(torch.nn.Module):
    def __init__(self, feat_sizes, dim_config, hidden_size):
        ...
        input_size = (
            hidden_size
            + (dim_config["motif_seq_positional_dim"] + dim_config["atom_id_dim"] + dim_config["atom_charge_dim"])
            * 2
        )
    ...
    def inference(
        self, shape_embeddings, batch, dataset, node_featurizer, motif_atom_decoder, atom_idx, atom_charges
    ):
